<template>
  <div class="pybossa-options is-outilineness" tabindex="0" v-on:keyup="checkKeyCode" style="padding-top:1.8rem;">
    <p v-if="multi" class="title is-6">{{$t('optionlist.choose')}}</p>
    <template v-for="(option, index) in options">
      <div v-bind:key="index" class="box"
           :class="{'is-success animated pulse': isSelected(option.activationKey)}"
          ...
          <article class="media">
            <div class="media-left">
              <b-tooltip :active="isTooltipActive(option.activationKey)" always :label="$t('optionlist.press')" position="is-top">
                <span class="button is-capitalized" :class="{'has-text-weight-semibold is-green is-outlined': isSelected(option.activationKey)}" style="font-size:12px;">{{option.activationKey}}</span>
              </b-tooltip>
            </div>
            <div class="media-content">
              <div class="content">
                <template v-if="option.hasOwnProperty('other') && isSelected(option.activationKey)">
                  <b-input v-model="other"></b-input>
                </template>
                <template v-else>
                  <p :class="{'has-text-weight-semibold': isSelected(option.activationKey)}">{{option.text}}</p>
                  <p v-if="option.extra">{{option.extra}}</p>
                </template>
              </div>
            </div>
            <div v-if="multi" class="media-right">
              <b-icon v-if="multi && isSelected(option.activationKey)" class="is-pulled-right" icon="check"></b-icon>
            </div>
          </article>
      </div>
    </template>
  ...
  methods: {
    ...
    saveValue (option) {
      ...
      if (this.multi) {
        ...
        if (found.hasOwnProperty('storeValue') && found.hasOwnProperty('other') === false) {
        ...
        if (this.multi) {
          if (this.multiSelected.indexOf(activationKey) !== -1) {
            if (found.hasOwnProperty('other') === false) {
              this.multiSelected = _.remove(this.multiSelected, function (n) { return n !== activationKey })
              let d = _.cloneDeep(this.storeValue)
              _.remove(d, function (n) { return n === found.storeValue })
              ...
              if (this.other !== '') {
                ...
                this.multiSelected = _.remove(this.multiSelected, function (n) { return n !== activationKey })
                let d = _.cloneDeep(this.storeValue)
                _.remove(d, function (n) { return n === o })
                this.$emit('update:storeValue', d)
                del = true
                this.other = ''
              }
      ...
      if (tmp !== undefined) {
        if (this.multi) {
          if (tmp.hasOwnProperty('storeValue')) {
            ...
            _.remove(data, function (n) { return n === oldVal })
            _.remove(data, function (n) { return n === newVal })
            _.remove(data, function (n) { return n === '' })
            if (newVal !== '') {
              data.push(newVal)
            }
